      ...
      >
        <DialogTitle sx={{ 
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            >
              <AutoAwesomeIcon sx={{ fontSize: 28, color: '#3b82f6' }} />
            </Box>
            <Box>
              ...
              <Typography variant="body2" sx={{ color: 'rgba(255, 255, 255, 0.7)' }}>
                Add details for your new design group
              </Typography>
            </Box>
          </Box>
        </DialogTitle>
        ...
        <DialogContent sx={{ p: 4 }}>
          ...
          <Box sx={{ 
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
              {selectedProducts.slice(0, 5).map((_, index) => (
                <Box
                  key={index}
                  sx={{
                    width: 12,
          ...
          <Box sx={{ mb: 4 }}>
            ...
            <TextField
              placeholder="Enter a descriptive name for your design group"
              value={newGroupName}
          ...
          <Box sx={{ 
            ...
            <Typography variant="body2" sx={{ color: 'rgba(255, 255, 255, 0.7)', mb: 2 }}>
              Add tags to categorize and organize your design group
            </Typography>
              ...
              {newGroupTags.length === 0 ? (
                ...
                newGroupTags.map((tag, index) => (
                  <Chip
                    key={index}
                    label={tag}
                    onDelete={isCreatingGroup ? undefined : () => {
            ...
            {newGroupTags.length < 10 && (
              <Box sx={{ display: 'flex', gap: 1 }}>
                <TextField
                  placeholder="e.g., seasonal, promotional, bestseller"
                  value={newGroupTag}
                  ...
                  disabled={isCreatingGroup}
                  onKeyPress={(e) => {
                    if (e.key === 'Enter' && newGroupTag.trim() && !newGroupTags.includes(newGroupTag.trim()) && !isCreatingGroup) {
                ...
                <Button
                  variant="outlined"
                  onClick={() => {
                    if (newGroupTag.trim() && !newGroupTags.includes(newGroupTag.trim()) && !isCreatingGroup) {
                      setNewGroupTags(prev => [...prev, newGroupTag.trim()]);
                      setNewGroupTag('');
                    }
                  }}
                  disabled={!newGroupTag.trim() || newGroupTags.includes(newGroupTag.trim()) || isCreatingGroup}
                  sx={{ 
                    borderColor: '#3b82f6',
                    color: '#3b82f6',
                    '&:hover': { borderColor: '#2563eb', bgcolor: 'rgba(59, 130, 246, 0.1)' },
          ...
          {/* Enhanced Search Keywords Section with AI Suggestions */}
          <Box sx={{ 
            p: 3, 
            border: '1px solid rgba(34, 197, 94, 0.2)', 
            borderRadius: 2, 
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
              ...
              {loadingSuggestions && (
                <CircularProgress size={16} sx={{ color: '#22c55e', ml: 1 }} />
              )}
            </Box>
            <Typography variant="body2" sx={{ color: 'rgba(255, 255, 255, 0.7)', mb: 2 }}>
              Help customers find this group by adding relevant search terms
            </Typography>
            ...
            {newGroupSearchKeywords.length < 20 && (
              <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
                <TextField
                  placeholder="e.g., vinyl wrap, car decal, custom design"
                  ...
                  disabled={isCreatingGroup}
                  onKeyPress={(e) => {
                    if (e.key === 'Enter' && newGroupSearchKeyword.trim() && !newGroupSearchKeywords.includes(newGroupSearchKeyword.trim().toLowerCase()) && !isCreatingGroup) {
                ...
                <Button
                  variant="outlined"
                  onClick={() => {
                    if (newGroupSearchKeyword.trim() && !newGroupSearchKeywords.includes(newGroupSearchKeyword.trim().toLowerCase()) && !isCreatingGroup) {
                      setNewGroupSearchKeywords(prev => [...prev, newGroupSearchKeyword.trim().toLowerCase()]);
                      setNewGroupSearchKeyword('');
                    }
                  }}
                  disabled={!newGroupSearchKeyword.trim() || newGroupSearchKeywords.includes(newGroupSearchKeyword.trim().toLowerCase()) || isCreatingGroup}
                  sx={{ 
                    borderColor: '#22c55e',
                    color: '#22c55e',
                    '&:hover': { borderColor: '#16a34a', bgcolor: 'rgba(34, 197, 94, 0.1)' },
                ...
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                  {aiSuggestions.map((suggestion, index) => (
                    <Chip
                      ...
                      disabled={newGroupSearchKeywords.includes(suggestion) || isCreatingGroup}
                      size="small"
                      sx={{
                        bgcolor: newGroupSearchKeywords.includes(suggestion) 
                          ? 'rgba(255, 255, 255, 0.1)' 
                          : 'rgba(139, 92, 246, 0.2)',
                        color: newGroupSearchKeywords.includes(suggestion) 
                          ? 'rgba(255, 255, 255, 0.5)' 
                          : '#8b5cf6',
                        border: '1px solid rgba(139, 92, 246, 0.3)',
        ...
        <DialogActions sx={{ 
          ...
          <Button
            ...
            disabled={!newGroupName.trim() || newGroupName.length > 200 || isCreatingGroup}
            sx={{

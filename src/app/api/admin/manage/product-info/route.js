 ...
 *
 * Creates a new product info document.
 *
 * Expected JSON payload:
 * {
 *   content: { ... }, // Editor.js content JSON
 *   product: "productId" (optional),
 *   specificCategoryVariant: "variantId" (optional),
 *   specificCategory: "categoryId" (optional),
 *   // Other fields if required (e.g., title, images, etc.)
 * }
 */
export async function POST(request) {
  try {
    ...
    // Create new document
    const productInfo = new ProductInfoTab(body);
    await productInfo.save();

    return new Response(JSON.stringify(productInfo), { status: 201 });
  } catch (error) {
    console.error("Error in POST /api/productinfo:", error);
    return new Response(JSON.stringify({ error: error.message }), { status: 500 });
  }
}

...
export default function Goals({ goals, setGoals, isAllowed, department }) {
  ...
  const { user } = useUser();
  const canReorder = user?.primaryEmailAddress?.emailAddress === "priyanshuyadav0404@gmail.com" || 
                    user?.primaryEmailAddress?.emailAddress === "luckysolanki902@gmail.com";
  ...
  return (
    <Paper elevation={2} sx={{ borderRadius: 3, bgcolor: "background.paper", overflow: "hidden" }}>
      <DragDropContext onDragEnd={handleDragEnd}>
        <Droppable droppableId="goals-list" isDropDisabled={!canReorder || isReordering}>
          {(provided, snapshot) => (
            <List 
              ...
              {goals.map((goal, index) => {
                ...
                return (
                  <Draggable 
                    ...
                    index={index}
                    isDragDisabled={!canReorder || isReordering || goal.isCompleted}
                  >
                    {(provided, snapshot) => (
                      <Box
                        ref={provided.innerRef}
